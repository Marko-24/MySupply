...
export default function BoxDetailsScreen() {
    ...
    // Dynamic params fallback
    const box = {
        id: (params.id as string) || "a1b2c3d4-e5f6-7890-abcd-1234567890ab",
        vendor_id: (params.vendor_id as string) || "11111111-1111-1111-1111-111111111111",
    ...
    const handleAddToCartOnly = () => {
        ...
        Alert.alert(
            ...
            "Производот е додаден. Сакате ли да ја отворите кошничката?",
            [
                { text: "Остани тука", style: "cancel" },
                { text: "Оди во кошничка", onPress: () => router.push("/cart" as any) },
            ]
        );
    };
    ...
    const handleProcessPayment = async () => {
        ...
        try {
            ...
            if (insertError) {
                console.error("Database Order Insert Error:", insertError);
                Alert.alert("Грешка во база", insertError.message);
            } else {
                setPickupCode(code);
                setIsPaymentModalVisible(false);
                Alert.alert(
                    "Успешна резервација!",
                    `Твојата нарачка е креирана! Код: ${code}`,
                    [{ text: "Види во Нарачки", onPress: () => router.push("/orders" as any) }]
                );
            }
        } catch (err: any) {
        ...
        <ScreenContainer>
            <View style={styles.mainContainer}>
                ...
                <View style={styles.bottomBar}>
                    <TouchableOpacity style={styles.cartIconBtn} onPress={handleAddToCartOnly}>
                        <Text style={{ fontSize: 20 }}>🛒</Text>
                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    priceContainer: { gap: 2, flex: 1 },
    totalPriceLabel: { fontSize: 11, color: "#6B7280" },
    priceRow: { flexDirection: "row", alignItems: "baseline", gap: 6 },
    currentPrice: { fontSize: 18, fontWeight: "900", color: "#111827" },
    oldPrice: { fontSize: 12, color: "#9CA3AF", textDecorationLine: "line-through" },
    woltOrderBtn: { backgroundColor: "#00C2E0", paddingVertical: 12, paddingHorizontal: 18, borderRadius: 12 },
    woltOrderBtnText: { color: "#FFFFFF", fontSize: 14, fontWeight: "800" },
