...
export default function ThemeLabScreen() {
    ...
    const { colorScheme = "light", setColorScheme } = useThemeContext() || {};
    const colors = useColors();

    // Безбедно извлекување на имињата на палетата внатре во компонентата
    const paletteNames = useMemo(() => {
        return SchemeColors?.light ? (Object.keys(SchemeColors.light) as PaletteName[]) : [];
    }, []);

    const currentSchemeColors = SchemeColors?.[colorScheme] ?? SchemeColors?.light ?? {};

    const swatches = useMemo(
        () =>
            paletteNames.map((name) => ({
                name,
                value: currentSchemeColors[name] ?? "#000000",
            })),
        [colorScheme, paletteNames, currentSchemeColors],
    );

    const tileStyles = useMemo(() => {
        const build = (scheme: ColorScheme) => {
            const target = SchemeColors?.[scheme] ?? SchemeColors?.light ?? {};
            return {
                background: target.background ?? "#ffffff",
                border: target.border ?? "#e5e5e5",
                text: target.foreground ?? "#000000",
                subText: target.muted ?? "#888888",
                activeBackground: target.primary ?? "#44b273",
                activeText: target.background ?? "#ffffff",
            };
        };
        return {
            light: build("light"),
                    ...
                    <ThemedView className="rounded-2xl border border-border p-4">
                        ...
                        <View className="mt-4 flex-row flex-wrap gap-2">
                            <TouchableOpacity
                                ...
                                style={{ backgroundColor: currentSchemeColors.primary }}
                                onPress={() => {
                            ...
                            <TouchableOpacity
                                ...
                                style={{ backgroundColor: currentSchemeColors.surface }}
                                onPress={() => {
                                    setPressCount((count) => count + 1);
                            ...
                            <TouchableOpacity
                                className="rounded-full px-4 py-2"
                                style={{ backgroundColor: currentSchemeColors.success }}
                                onPress={() => {
                                    setPressCount((count) => count + 1);
                            ...
                            <TouchableOpacity
                                className="rounded-full px-4 py-2"
                                style={{ backgroundColor: currentSchemeColors.warning }}
                                onPress={() => {
                                    setPressCount((count) => count + 1);
                            ...
                            <TouchableOpacity
                                className="rounded-full px-4 py-2"
                                style={{ backgroundColor: currentSchemeColors.error }}
                                onPress={() => {
                                    setPressCount((count) => count + 1);
                        ...
                        <View className="mt-4 rounded-xl bg-background p-4 border border-border">
                            ...
                            <View className="mt-3 gap-2">
                                <View className="flex-row items-center gap-2">
                                    <IconSymbol name="house.fill" color={colors?.tint ?? "#000"} size={20} />
