...
export default function RegisterScreen() {
    ...
    async function handleGoogleRegister() {
        try {
            ...
            // 1. Креирање на правилен redirectUri за Expo Go
            ...
            // 2. Барање за OAuth URL од Supabase со експлицитно redirectTo
            ...
            if (data?.url) {
                // 3. Извршување на WebBrowser сесијата
                const result = await WebBrowser.openAuthSessionAsync(data.url, redirectUri);

                if (result.type === 'success' && result.url) {
                    // Извлекување токени од хешот (#access_token=...) или query-то (?access_token=...)
                    const responseUrl = result.url;
                    const hashIndex = responseUrl.indexOf('#');
                    const searchIndex = responseUrl.indexOf('?');

                    let paramsString = '';
                    if (hashIndex !== -1) {
                        paramsString = responseUrl.substring(hashIndex + 1);
                    } else if (searchIndex !== -1) {
                        paramsString = responseUrl.substring(searchIndex + 1);
                    }
